...
Electro is a highly modular framework; as such, even your application is a module (or several modules). Modules may be private, plugins or templates.
...
To install or remove plugins, templates and other Composer packages, you should use the relevant `workman` commands (see below).
...
> Instead of registering dependencies on the `composer.root.json` file, we recommend that you do not modify that file and, instead, always register dependencies on the modules that require them. That way, your application will be more modular and the `workman install/remove` commands will work as intedend.
...
Before you can start developing your application, you must create, at least, one module for it.
...
Alternatively, if you intend to use the Matisse templating engine for creating a website or a web application, you may use this command instead:
